...
function UpdateStatusModal({ open, onClose, onSave, vehicle }) {
  const [status, setStatus] = useState(vehicle?.status === "Available" ? "Available" : "Out of Service");
  if (!open) return null;
...
function VehicleCard({ vehicle, onRecordRepair, onUpdateStatus }) {
  ...
  const hoverButton = { ...button, background: "#333", color: "white" };
  return (
    <div style={card}>
      <div style={header}>
...
export default function ShopModule() {
  ...
  const notifications = ["Vehicle PW-1 is scheduled for repair on 06/23/2025","New repair request submitted for LE-4"];
  ...
  const filteredVehicles = vehicles.filter(v => v.name.toLowerCase().includes(filter.toLowerCase()) || v.plate.toLowerCase().includes(filter.toLowerCase()) || v.status.toLowerCase().includes(filter.toLowerCase()));
